...
uint8_t JPEGDecoder::pjpeg_need_bytes_callback(uint8_t* pBuf, uint8_t buf_size, uint8_t *pBytes_actually_read, void *pCallback_data) {
	...
	if (jpg_source == JPEG_ARRAY) { // We are handling an array
		for (int i = 0; i < n; i++) {
			//MJE pBuf[i] = pgm_read_byte(jpg_data++);
	...
	res = f_read(&g_pInFileSd, pBuf, n, &br);
...
int JPEGDecoder::decodeSdFile(const char *pFilename) {
	...
	FIL pInFile;
	f_open(&pInFile, pFilename, FA_OPEN_EXISTING | FA_READ);
...
int JPEGDecoder::decodeSdFile(FIL jpgFile) { // This is for the SD library
	...
	g_nInFileSize = g_pInFileSd.fsize;
